...
void Settings_draw()
{
	...
	if (g_Lang == "pol")
		Menu_options = {"English", "Polski", "30", "60", "144", "Bez limitu", "Powrót do Menu"};
	else
		Menu_options = {"English", "Polski", "30", "60", "144", "Unlimited", "Back to Menu"};

	// Zmienne
	g_SettingsButtons.resize(Menu_options.size());
	int buttonWidth = 750;
	...
	// Sprawdzanie wybranych opcji (ustawienia)
	for (int i=0; i<g_SettingsButtons.size(); i++)
		g_SettingsButtons[i].selected = false;

	// Jêzyk
	if (g_Lang == "pol")
		g_SettingsButtons[1].selected = true;
	else
		g_SettingsButtons[0].selected = true;

	// FPS
	if (g_TargetFPS == 30)
		g_SettingsButtons[2].selected = true;
	else if (g_TargetFPS == 60)
		g_SettingsButtons[3].selected = true;
	else if (g_TargetFPS == 144)
		g_SettingsButtons[4].selected = true;
	else if (g_TargetFPS == LLONG_MAX)
		g_SettingsButtons[5].selected = true;
	...
	for (int i=0; i<g_SettingsButtons.size(); i++)
	{
		...
		// Nadaj w³aœciowoœci
		g_SettingsButtons[i].frame = {startX, startY + i * (buttonHeight+buttonSpacing), buttonWidth, buttonHeight};
		g_SettingsButtons[i].rect = {startX + 10, startY + i*(buttonHeight+buttonSpacing) + 10, buttonWidth - 10, buttonHeight - 10};
		g_SettingsButtons[i].text = Menu_options[i];

		// Narysuj przycisk
		Draw_button(g_SettingsButtons[i]);
	}
}
...
void Settings_event(SDL_Event event, bool &quit, bool &error)
{
	if (event.type == SDL_MOUSEBUTTONDOWN) // Klikanie
	{
		...
		for (int i=0; i<g_SettingsButtons.size(); i++)
		{
			if (SDL_PointInRect(&Point, &g_SettingsButtons[i].frame) == true)
			{
				if (DEVELOPER_MODE == true)
					cout<<"Clicked on: '"<<g_SettingsButtons[i].text<<"'"<<endl;
	...
	else if (event.type == SDL_MOUSEMOTION) // Rodzaj kursora
	{
		...
		for (int i=0; i<g_SettingsButtons.size(); i++)
			g_SettingsButtons[i].hovered = false;

		for (int i=0; i<g_SettingsButtons.size(); i++)
		{
			if (SDL_PointInRect(&Point, &g_SettingsButtons[i].frame) == true)
			{
				g_SettingsButtons[i].hovered = true;
