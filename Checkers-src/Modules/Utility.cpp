...
void Draw_button(Button &button)
{
	...
	// SprawdŸ czy jest to wybrana opcja (ustawienia)
	if (button.selected == true)
...
void Draw_text(string txt, int x, int y, string pos, TTF_Font* Font, SDL_Color TxtColor, bool DrawBg)
{
	...
	if (DrawBg == true)
		textSurface = TTF_RenderUTF8_Shaded(Font, txt.c_str(), TxtColor, COLOR_BLACK);
	else
		textSurface = TTF_RenderUTF8_Blended(Font, txt.c_str(), TxtColor);
...
bool Confirm_request(string line1, string line2)
{
	...
	SDL_Window* Temp_Window = SDL_CreateWindow(line1.c_str(), SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 800, 450, SDL_WINDOW_SHOWN | SDL_WINDOW_BORDERLESS);
	...
	if (g_Lang == "pol")
		Menu_options = {"Tak", "Nie"};
	else
		Menu_options = {"Yes", "No"};

	// Zmienne
	vector<Button> ConfirmButtons(Menu_options.size());
	int buttonWidth = 500;
	...
	while (!quit)
	{
		...
		while (SDL_PollEvent(&event))
		{
			if (event.type == SDL_MOUSEBUTTONDOWN) // Klikanie
			{
				...
				{
					if (SDL_PointInRect(&Point, &ConfirmButtons[i].frame) == true)
					{
						if (DEVELOPER_MODE == true)
							cout<<"Clicked on: '"<<ConfirmButtons[i].text<<"'"<<endl;
			...
			else if (event.type == SDL_MOUSEMOTION) // Rodzaj kursora
			{
				...
				for (int i=0; i<ConfirmButtons.size(); i++)
					ConfirmButtons[i].hovered = false;

				for (int i=0; i<ConfirmButtons.size(); i++)
				{
					if (SDL_PointInRect(&Point, &ConfirmButtons[i].frame) == true)
					{
						ConfirmButtons[i].hovered = true;
		...
		{
			// Nadaj w³aœciowoœci
			ConfirmButtons[i].frame = {startX + i*(buttonWidth+buttonSpacing), startY, buttonWidth, buttonHeight};
			ConfirmButtons[i].rect = {startX + i*(buttonWidth+buttonSpacing) + 10, startY + 10, buttonWidth - 10, buttonHeight - 10};
			ConfirmButtons[i].text = Menu_options[i];

			// Narysuj przycisk
			Draw_button(ConfirmButtons[i]);
		}
...
void Debug()
{
	...
	cout<<"g_VersusAI = "<<g_VersusAI<<endl;
